...
#    `functionName` is a function's name in your module (e.g., `LandR_BiomassGMCCEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/LBMR_ClimateSensitive/LandR_BiomassGMCC/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandR_BiomassGMCC")
  ...
  parameters <- list(
    ...
    LandR_BiomassGMCC = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandR_BiomassGMCCEvent1", envir = .GlobalEnv)) {
    simOutput <- LandR_BiomassGMCCEvent1(mySim)
  } else {
    simOutput <- mySim$LandR_BiomassGMCCEvent1(mySim)
  }
  ...
  if (exists("LandR_BiomassGMCCEvent2", envir = .GlobalEnv)) {
    simOutput <- LandR_BiomassGMCCEvent2(mySim)
  } else {
    simOutput <- mySim$LandR_BiomassGMCCEvent2(mySim)
  }
